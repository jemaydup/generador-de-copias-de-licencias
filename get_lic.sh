...
if [ -d "$FolderData" ]; then
    ...
    echo -e $(date +'%Y-%m-%d--%H%M%S') "# seteamos los equipos a usar: $lista" >> $folder/log/$dia.log
    Gstart=$(date +%s)
